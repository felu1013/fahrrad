...
import { type Fahrrad, modelName } from '../../fahrrad/entity/index.js';
...
export class DbPopulateService implements OnApplicationBootstrap {
    ...
    readonly #fahrradModel: mongoose.Model<Fahrrad>;
    ...
    /**
     * Initialisierung durch DI mit `Model<Fahrrad>` gemäß _Mongoose_.
     */
    constructor(
        dbService: DbService,
        @InjectModel(modelName) fahrradModel: mongoose.Model<Fahrrad>,
    ) {
    ...
    async #populateTestdaten() {
        ...
        try {
            await this.#fahrradModel.collection.drop();
        } catch {
            this.#logger.info('#populateTestdaten: Keine Collection vorhanden');
        }

        const collection = await this.#fahrradModel.createCollection();
        this.#logger.warn(
        ...
        const insertedDocs = await this.#fahrradModel.insertMany(
            this.#testdaten,
            {
                lean: true,
            },
